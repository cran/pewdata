...
When used interactively, the `pew_download` function will be ask for the contact information requested by Pew of those wishing to download its datasets: name, organization, phone, and email. 
...
The `pew_download` function (1) simulates a visit to the website of the specified Pew dataset, (2) enters the required contact information and agrees to the terms and conditions of the download, (3) downloads the dataset's files, and, optionally but by default, (4) decompresses the dataset's files to a subdirectory within a specified directory (or, by default, a `pew_data` directory in the current working directory).
